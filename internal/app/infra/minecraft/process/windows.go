...
func NewWindowsProcess(cfg ProcessConfig) Process {
	p := &WindowsProcess{
		execFile:     path.Join(cfg.RootDir, "bedrock_server.exe"),
		runFile:      path.Join(cfg.RootDir, fmt.Sprintf("bedrock_server_%s.exe", cfg.ID)),
		checkRunFile: true,
		logFile:      cfg.LogFile,
	}
...
func (p *WindowsProcess) findExistingProcess() int {
	cmd := exec.Command("tasklist", "/FI", fmt.Sprintf("IMAGENAME eq %s", path.Base(p.runFile)), "/FO", "CSV", "/NH")
...
func (p *WindowsProcess) attachToProcess() {
	if p.pid == 0 {
		return
	}
	...
	// 尝试获取标准输入
	stdin, err := os.OpenFile(fmt.Sprintf("\\\\.\\pipe\\bedrock_server_%d", p.pid), os.O_WRONLY, 0)
	if err == nil {
		p.stdin = stdin
	} else {
...
func (p *WindowsProcess) Exist() bool {
	return utils.FileExist(p.execFile)
}

// checkProcessActive 检查进程是否在运行
func (p *WindowsProcess) checkProcessActive(pid int) bool {
...
func (p *WindowsProcess) Active() bool {
	// 如果进程ID为0，说明进程未启动
	if p.pid == 0 {
		return false
	}
...
func (p *WindowsProcess) Restart() error {
	err := p.Stop()
	if err != nil {
		return err
	}
...
func (p *WindowsProcess) ExecCmd(arg ...string) error {
	if !p.Active() {
		return errors.New("server process is not running")
	}
...
func (p *WindowsProcess) ScanLog(line int) (string, error) {
	// 参数验证
	if line <= 0 {
