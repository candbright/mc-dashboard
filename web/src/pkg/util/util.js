...
const DataUtils = {
   ...
   */
  convertToString(data, options = { recursive: true, skipNull: true }) {
    const processor = value => {
      if (TypeUtils.isNumber(value) || TypeUtils.isBoolean(value)) return value.toString()
      if (options.recursive && TypeUtils.isObject(value)) {
        return Array.isArray(value)
          ? value.map(processor)
          : this.convertToString(value, options)
      }
      return value
    }
